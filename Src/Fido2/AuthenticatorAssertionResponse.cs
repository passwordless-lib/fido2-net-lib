...
public sealed class AuthenticatorAssertionResponse : AuthenticatorResponse
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (AttestationObject is not null)
        {
            ...
            (var attType, var trustPath) = await verifier.VerifyAsync(attStmt, AuthenticatorData, hash);
            ...
            else if (matchedDpkRecords.Count is 1)
            {
                ...
                else
                {
                    ...
                    try
                    {
                        ...
                        _ = await verifier.VerifyAsync(devicePublicKeyAuthenticatorOutput.AttStmt, devicePublicKeyAuthenticatorOutput.GetAuthenticatorData(), devicePublicKeyAuthenticatorOutput.GetHash());
                    }
                    catch (Exception ex)
                    {
                        // Some form of error has occurred. It is indeterminate whether this is a known device. Terminate these verification steps.
