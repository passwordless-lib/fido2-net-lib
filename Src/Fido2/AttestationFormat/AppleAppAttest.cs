...
internal sealed class AppleAppAttest : AttestationVerifier
{
    public static byte[] GetAppleAppIdFromCredCertExtValue(X509ExtensionCollection exts)
    {
        ...
        if (appleExtension is null || appleExtension.RawData is null)
            throw new Fido2VerificationException(Fido2ErrorCode.InvalidAttestation, "Extension with OID 1.2.840.113635.100.8.5 not found on Apple AppAttest credCert");
        ...
        throw new Fido2VerificationException(Fido2ErrorCode.InvalidAttestation, "Apple AppAttest attestation extension 1.2.840.113635.100.8.5 has invalid data");
    }
    ...
    public override async ValueTask<VerifyAttestationResult> VerifyAsync(VerifyAttestationRequest request)
    {
        ...
        {
            throw new Fido2VerificationException(Fido2ErrorCode.InvalidAttestation, "Malformed x5c in Apple AppAttest attestation");
        }
        ...
        {
            throw new Fido2VerificationException(Fido2ErrorCode.InvalidAttestation, "Public key hash does not match key identifier in Apple AppAttest attestation");
        }
        ...
        {
            throw new Fido2VerificationException(Fido2ErrorCode.InvalidAttestation, "App ID hash does not match RP ID hash in Apple AppAttest attestation");
        }

        // 7. Verify that the authenticator data's counter field equals 0.
        if (request.AuthData.SignCount != 0)
        {
            throw new Fido2VerificationException(Fido2ErrorCode.InvalidAttestation, "Sign count does not equal 0 in Apple AppAttest attestation");
        }
        ...
        {
            throw new Fido2VerificationException(Fido2ErrorCode.InvalidAttestation, "Invalid aaguid encountered in Apple AppAttest attestation");
        }

        // 9. Verify that the authenticator data's credentialId field is the same as the key identifier.
        if (!keyIdentifier.SequenceEqual(request.AuthData.AttestedCredentialData.CredentialId))
        {
            throw new Fido2VerificationException(Fido2ErrorCode.InvalidAttestation, "Mismatch between credentialId and keyIdentifier in Apple AppAttest attestation");
        }
