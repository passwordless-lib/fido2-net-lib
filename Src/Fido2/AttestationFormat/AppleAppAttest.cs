...
namespace Fido2NetLib
{
    ...
    {
        public static byte[] GetAppleAppIdFromAttestationExtensionValue(X509ExtensionCollection exts)
        {
        ...
        internal static Guid FromBigEndian(byte[] Aaguid)
        {
            (Aaguid[0], Aaguid[3]) = (Aaguid[3], Aaguid[0]);
            (Aaguid[1], Aaguid[2]) = (Aaguid[2], Aaguid[1]);
            (Aaguid[4], Aaguid[5]) = (Aaguid[5], Aaguid[4]);
            (Aaguid[6], Aaguid[7]) = (Aaguid[7], Aaguid[6]);
        ...
        public override (AttestationType, X509Certificate2[]) Verify()
        {
            ...
            // We are not quite done with step 1 yet, we'll finish that later
            // We'll skip steps 2 - 4 for now, and handle those at the end
            ...
            var appId = GetAppleAppIdFromAttestationExtensionValue(credCert.Extensions);
            ...
            else if (!AuthData.AttestedCredentialData.AaGuid.Equals(prodAaguid))
            {
            ...
            Apple apple = new();
            return apple.Verify(attStmt, authenticatorData, clientDataHash);
        }
    }
}
