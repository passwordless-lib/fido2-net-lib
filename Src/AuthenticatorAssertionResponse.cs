    ...
    public class AuthenticatorAssertionResponse : AuthenticatorResponse
    {
        ...
        public async Task<AssertionVerificationResult> VerifyAsync(AssertionOptions options, string expectedOrigin, byte[] storedPublicKey, uint storedSignatureCounter, IsUserHandleOwnerOfCredentialIdAsync isUserHandleOwnerOfCredId, byte[] requestTokenBindingId)
        {
            ...
            using (var sha = SHA256.Create())
            {
                ...
                var rpid = Raw.Extensions.AppID ? options.Extensions.AppID : options.RpId;
                hashedRpId = sha.ComputeHash(Encoding.UTF8.GetBytes(rpid));
                // 15
            ...
            if ((true == authData.Flags.HasFlag(AuthenticatorFlags.ED)) && ((null == authData.Extensions) || (0 == authData.Extensions.Length))) throw new Fido2VerificationException("Extensions flag present, malformed extensions detected");
            if ((false == authData.Flags.HasFlag(AuthenticatorFlags.ED)) && (null != authData.Extensions)) throw new Fido2VerificationException("Extensions flag not present, but extensions detected");
